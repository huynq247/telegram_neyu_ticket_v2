...
Handle user authentication using Odoo's XML-RPC API
"""
...
class OdooAuthService:
    ...
    def _try_xmlrpc_auth(self, email: str, password: str) -> Tuple[bool, Optional[Dict[str, Any]], Optional[str]]:
        ...
        return False, None, last_error
            ...
            logger.info(f"Authentication successful for user: {email} (UID: {uid})")
            return True, formatted_user, None
            
        except xmlrpc.client.Fault as e:
            logger.error(f"Odoo XML-RPC error during authentication: {e}")
    ...
    def create_session(self, telegram_user_id: int, odoo_user_data: Dict[str, Any]) -> str:
        ...
        logger.info(f"Created session for Telegram user {telegram_user_id}, Odoo user {odoo_user_data['email']}")
    ...
    def revoke_session(self, telegram_user_id: int) -> bool:
        ...
        if telegram_user_id in self.active_sessions:
            user_email = self.active_sessions[telegram_user_id]['odoo_user']['email']
            del self.active_sessions[telegram_user_id]
            logger.info(f"Revoked session for Telegram user {telegram_user_id}, Odoo user {user_email}")
            return True
        
        return False
    
    def get_user_permissions(self, telegram_user_id: int) -> Dict[str, bool]:
        """
        Get user permissions based on Odoo groups
        ...
        return {
            ...
            'can_view_all_tickets': user_data.get('is_helpdesk_manager', False),
            'can_manage_tickets': user_data.get('is_helpdesk_manager', False),
            'is_helpdesk_manager': user_data.get('is_helpdesk_manager', False)
        }
    
    def _is_helpdesk_manager(self, groups: list) -> bool:
        """Check if user is helpdesk manager"""
        manager_groups = ['Helpdesk Manager', 'Administration / Settings']
        return any(group['name'] in manager_groups for group in groups)
    
    def _is_helpdesk_user(self, groups: list) -> bool:
        """Check if user is helpdesk user"""
        user_groups = ['Helpdesk User', 'User types / Internal User']
        return any(group['name'] in user_groups for group in groups)
    ...
    def cleanup_expired_sessions(self) -> int:
        ...
        for telegram_user_id, session in self.active_sessions.items():
            if now > session['expires_at']:
                expired_sessions.append(telegram_user_id)
        
        for telegram_user_id in expired_sessions:
            self.revoke_session(telegram_user_id)
